...
#define RX_SIZE 512
#define MESH_BOOT_RETRY_MS 10000U
#define ROLLBACK_HEALTH_MS 20000U
...
#ifdef CONFIG_MIXER_MESH_RELAY_ELIGIBLE
#define MIXER_RELAY_ELIGIBLE true
#else
#define MIXER_RELAY_ELIGIBLE false
#endif
...
static const char *TAG = "esp_mixer";
...
static bool s_mesh_netifs_created;
...
static esp_err_t configure_mesh(void)
{
	...
	if ((err = esp_mesh_set_topology(MESH_TOPO_TREE)) != ESP_OK) return err;
	...
	if ((err = esp_mesh_set_ap_authmode(WIFI_AUTH_WPA2_PSK)) != ESP_OK) return err;
...
static void mark_rollback_valid_when_healthy(void)
{
	if (!s_rollback_pending ||
	    (uint32_t)(tick_ms() - s_app_started_ms) < ROLLBACK_HEALTH_MS) return;
	if (!display_controller_ready() || !sensor_manager_ready()) return;
	esp_err_t err = esp_ota_mark_app_valid_cancel_rollback();
	if (err == ESP_OK) {
		s_rollback_pending = false;
		ESP_LOGI(TAG, "OTA rollback: running app marked valid after controller health gate");
	} else {
...
static void recovery_task(void *arg)
{
	...
	for (;;) {
		...
		if (mesh_v2_node_ack_fresh(ACK_FRESH_MS)) {
			...
			s_last_soft_reconnect_ms = 0;
			display_controller_set_mesh(true, true);
		...
		display_controller_set_mesh(true, false);
...
static void on_parent_connected(const mesh_event_connected_t *connected)
{
	...
	display_controller_set_mesh(true, false);
	display_controller_show_mesh_event("Mesh connected");
	update_topology();
	update_diagnostics();
			       ...
			       int32_t event_id, void *event_data)
{
	...
	case MESH_EVENT_STOPPED:
		...
		s_telemetry_synced = false;
		mesh_v2_link_parent_disconnected();
		display_controller_set_mesh(false, false);
		display_controller_show_mesh_event("Mesh stopped");
	...
	case MESH_EVENT_PARENT_DISCONNECTED: {
		...
		display_controller_set_mesh(false, false);
		display_controller_show_mesh_event("Parent lost");
	...
	case MESH_EVENT_LAYER_CHANGE:
		...
		{
			char notice[24];
			snprintf(notice, sizeof(notice), "Mesh layer %d", s_layer);
			display_controller_show_mesh_event(notice);
		}
		break;
...
static void detect_rollback_state(void)
{
	...
	if (running && esp_ota_get_state_partition(running, &state) == ESP_OK &&
		...
		ESP_LOGW(TAG, "OTA rollback pending until controller health gate");
	}
}

static esp_err_t initialize_mesh_runtime(void)
{
	esp_err_t err;
	...
	if (!s_runtime_initialized) {
		...
		err = mesh_v2_link_init(TAG, MIXER_RELAY_ELIGIBLE);
		if (err != ESP_OK) return err;
	...
	if (!s_rx_task_started) {
		...
		s_rx_task_started = true;
	}
	if (!s_recovery_task_started) {
		if (xTaskCreate(recovery_task, "mesh_recovery", 4096, NULL, 5, NULL) != pdPASS) {
			return ESP_ERR_NO_MEM;
		}
...
static void mesh_bootstrap_task(void *arg)
{
	(void)arg;
	for (;;) {
		esp_err_t err = initialize_mesh_runtime();
		if (err == ESP_OK) break;
		ESP_LOGE(TAG, "mesh bootstrap failed: %s; autonomous controller remains active",
			 esp_err_to_name(err));
		vTaskDelay(pdMS_TO_TICKS(MESH_BOOT_RETRY_MS));
	}

	const esp_partition_t *running = esp_ota_get_running_partition();
	ESP_LOGI(TAG,
		 "mesh ready idf=%s app=%s slot=%s flash=%s outputs=OFF relay=%u ps=%u",
		 esp_get_idf_version(), esp_app_get_description()->version,
		 running ? running->label : "?", CONFIG_ESPTOOLPY_FLASHSIZE,
		 MIXER_RELAY_ELIGIBLE ? 1U : 0U,
...
void app_main(void)
{
	...
	app_snapshot_init();
	err = display_controller_start();
	if (err != ESP_OK) {
		ESP_LOGW(TAG, "display unavailable: %s", esp_err_to_name(err));
	}
	err = command_adapter_start();
	if (err != ESP_OK) {
		ESP_LOGW(TAG, "event outbox unavailable: %s", esp_err_to_name(err));
	}
	err = sensor_manager_start();
	if (err != ESP_OK) {
		ESP_LOGW(TAG, "sensor manager unavailable: %s", esp_err_to_name(err));
	}
	err = gesture_controller_start();
	if (err != ESP_OK) {
		ESP_LOGW(TAG, "gesture controller unavailable: %s", esp_err_to_name(err));
	}

	if (s_rollback_pending &&
	    xTaskCreate(app_health_task, "app_health", 2048, NULL, 4, NULL) != pdPASS) {
		ESP_LOGE(TAG, "failed to create OTA health task");
	}
	if (xTaskCreate(mesh_bootstrap_task, "mesh_boot", 6144, NULL, 5, NULL) != pdPASS) {
		ESP_LOGE(TAG, "failed to create mesh bootstrap; autonomous controller remains active");
	}

	ESP_LOGI(TAG, "application ready; sensors/display run independently of mesh");
}
